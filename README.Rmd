...
--then, we activate the cache; although optional, it's highly recommended. Here, we choose a cache 
of max. 2 GB of memory ($2 \cdot 1024^3$ bytes)--`hrqolr` currently only allows in-memory caching.
...
`sample_example_trajectories` returns a `ggplot`, allowing you to fine-tune its appearance for your
...
wonky at end of follow-up due to increasingly few observations some of which might be low:
```{r}
