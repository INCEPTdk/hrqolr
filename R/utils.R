...
#' @name vapply_helpers
#'
vapply_num <- function(X, FUN, ...) {
	vapply(X, FUN, FUN.VALUE = numeric(1), ...)
}

#' @rdname vapply_helpers
#' @keywords internal
vapply_int <- function(X, FUN, ...) {
	vapply(X, FUN, FUN.VALUE = integer(1), ...)
}

#' @rdname vapply_helpers
#' @keywords internal
vapply_str <- function(X, FUN, ...) {
	vapply(X, FUN, FUN.VALUE = character(1), ...)
}

#' @rdname vapply_helpers
#' @keywords internal
vapply_lgl <- function(X, FUN, ...) { 
	vapply(X, FUN, FUN.VALUE = logical(1), ...)
}


#' Verify input is single integer or numeric (potentially within range) or string
#'
#' Used internally.
#'
